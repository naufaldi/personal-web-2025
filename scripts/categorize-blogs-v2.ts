...
function categorizePost(title: string, description: string): BlogCategory {
  ...
  if (journeyKeywords.some((keyword) => lowerTitle.includes(keyword))) {
    return "My journey";
  }

  // Check for opinion indicators
  if (opinionKeywords.some((keyword) => lowerTitle.includes(keyword))) {
    return "Opinions";
  }

  // Check for idea indicators
  if (ideaKeywords.some((keyword) => lowerTitle.includes(keyword))) {
    return "Idea";
  }

  // Check for technical indicators (more specific)
  if (technicalKeywords.some((keyword) => lowerTitle.includes(keyword))) {
  ...
  if (lowerDesc.includes("learn") || lowerDesc.includes("journey")) {
    return "My journey";
  }
...
function buildFrontmatter(data: BlogFrontmatter, newCategory: BlogCategory): string {
  // Use single quotes and proper YAML formatting
  const lines = [
    "---",
    ...
    `  avatar: "${data.author.avatar.replace(/"/g, '\\"')}"`,
    `date: ${data.date}`,
    `image: "${(data.image || "").replace(/"/g, '\\"')}"`,
    `canonical: "${(data.canonical || "").replace(/"/g, '\\"')}"`,
    "---",
    "",
  ];
  return lines.join("\n");
...
async function categorizeBlogPosts() {
  ...
  console.log("📚 Analyzing blog posts for categorization...\n");
  ...
  for (const file of files) {
    ...
    try {
      const parsed = fm<BlogFrontmatter>(content);
      const frontmatter = parsed.attributes;

      const newCategory = categorizePost(frontmatter.title, frontmatter.description);

      if (newCategory !== frontmatter.category) {
        // Build new frontmatter
        const newFrontmatter = buildFrontmatter(frontmatter, newCategory);
  ...
  console.log(`\n✨ Updated ${updated} posts, ${errors} errors`);
}

categorizeBlogPosts().catch((error) => {
