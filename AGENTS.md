...
- `bun run build`: Generate discoverability files, TypeScript check, and Vite production build
...
Static public files under `public/` plus metadata in `index.html` and route-level updates via `src/hooks/usePageMeta.ts`:
